/*
Lightweight Template Engine For NodeJS

Example

Template().render('test.html', {title: 123})
...
function include(fnz) {
    ...
    if(code === undefined) {
        ...
        //console.log(script);
        code = this.cache[fnz] = gVm.runInContext(script, this.parent._globalScope, {filename: fnz});
    }
...
function Template(root, debug, globalScope)
{
	if(!(this instanceof Template)) return new Template(root, debug, globalScope);
    ...
    this._globalScope.escapeHTML = escapeHTML;
	gVm.createContext(this._globalScope);
};
